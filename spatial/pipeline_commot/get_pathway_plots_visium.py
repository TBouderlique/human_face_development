...
save_dir = f"/home/felix/projects/facial/felix/plots/{sample}_ECM" # Switch here
...
f = open(f"{save_dir}/{sample}_all_pathways.txt", "w")
for path in paths:
    f.write(path)
    f.write("\n")
f.close()
...
for n, p in enumerate(paths):
    ...
    ax.set_title(p +' pathway', pad=0)   
    

    ax.legend_ = None
    
    
plt.suptitle(fontsize=20, t=f"Secreted Signaling receptor communication plots for the {sample}", y=.95) # Swtich here
plt.savefig(f"{save_dir}/{sample}_COMMOT_spatial_pathways.pdf",dpi=150)