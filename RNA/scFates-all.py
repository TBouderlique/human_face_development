...
adata.obs['Sample_type2'] = adata.obs['Sample_type']
adata.obs['Sample_type2'] = adata.obs['Sample_type2'].cat.add_categories(['sc'])
adata.obs.loc[adata.obs['Sample_type2'] != 'sn', 'Sample_type2'] = 'sc'

# Create a boolean mask to identify genes to remove
genes_to_remove = adata.var_names.str.startswith('MT-') | \
                  adata.var_names.str.startswith('RPS') | \
                  adata.var_names.str.startswith('RPL')
adata = adata[:, ~genes_to_remove].copy()


# Define origins to process
#origins = ['ear', 'eye', 'nose', 'jaw']
origins = ['nose']


# Filter mesenchymal clusters (shared across all origins)
mesenchyme_clusters = ['0','1','2','3','4','5','6','8','9','10','11','13']
adata_mes = adata[adata.obs['leiden'].isin(mesenchyme_clusters)].copy()
...
for origin in origins:
    ...
    data = adata_mes[adata_mes.obs['Origin'] == origin].copy()

    # Preprocessing
    scp.pp.normalize_total(data, target_sum=1e4)
    scp.pp.log1p(data)
    scp.pp.highly_variable_genes(data, min_mean=0.0125, max_mean=3, min_disp=0.5)
    ...
    cell_cycle_genes = [x for x in cell_cycle_genes.all_genes if x in data.var_names]

    data.raw = data.copy()
    scp.pp.scale(data)
    
    scp.tl.score_genes_cell_cycle(data, s_genes=s_genes, g2m_genes=g2m_genes)
    scp.pp.regress_out(data, ['S_score', 'G2M_score'])

    scp.tl.pca(data, n_comps=30, svd_solver='arpack', use_highly_variable=True)
    
    # Harmony integration (adjust if using harmonypy)
    ...
    scp.pp.neighbors(data, n_neighbors = 50, use_rep='X_pca_harmony')
    scp.tl.leiden(data, resolution=0.4)
    scp.tl.umap(data, n_components=2)

    # Plot and save
    scp.pl.umap(data, color=['leiden'], save=f'_{origin}_Sample_050725.png')
    data.write(f'{origin}_Sample.h5ad')

    print(f"✅ Finished {origin}\n")

print("🎉 All origins processed.")
...
# CytoTRACE
adata = scp.read_h5ad('ear_Sample1.h5ad')
...
scp.pl.umap(adata,color="ct_score", cmap="Spectral_r",  save = 'ear-cytoTRACE1.pdf', size = 10)
adata.write('ear_Sample1.h5ad')
...
# CytoTRACE
adata = scp.read_h5ad('eye_Sample1.h5ad')
scp.pl.umap(adata, color=['leiden'], legend_loc="on data", legend_fontsize = 10)
...
ckt = ctk.compute_cytotrace().compute_transition_matrix()
scp.set_figure_params(dpi=150)
scp.pl.umap(adata,color="ct_score", cmap="Spectral_r",  save = 'eye-cytoTRACE1.pdf', size = 10)
adata.write('eye_Sample1.h5ad')
...
ckt = ctk.compute_cytotrace().compute_transition_matrix()
scp.set_figure_params(dpi=150)
scp.pl.umap(adata,color="ct_score", cmap="Spectral_r",  save = 'nose-cytoTRACE1.pdf', size = 10)
adata.write('nose_Sample1.h5ad')
...
ckt = ctk.compute_cytotrace().compute_transition_matrix()
scp.set_figure_params(dpi=150)
scp.pl.umap(adata,color="ct_score", cmap="Spectral_r",  save = 'jaw-cytoTRACE1.pdf', size = 10)
adata.write('jaw_Sample1.h5ad')
...
scp.pl.embedding_density(adata, basis='umap', key = 'kdeAge', save = 'kde-ear-Age1.pdf')
...
adata = scp.read_h5ad('eye_Sample1.h5ad')
scp.tl.embedding_density(adata, basis='umap', groupby = 'Age', key_added = 'kdeAge')
scp.pl.embedding_density(adata, basis='umap', key = 'kdeAge', save = 'kde-eye-Age1.pdf')
...
adata = scp.read_h5ad('nose_Sample1.h5ad')
scp.tl.embedding_density(adata, basis='umap', groupby = 'Age', key_added = 'kdeAge')
scp.pl.embedding_density(adata, basis='umap', key = 'kdeAge', save = 'kde-nose-Age1.pdf')
...
adata = scp.read_h5ad('jaw_Sample1.h5ad')
scp.tl.embedding_density(adata, basis='umap', groupby = 'Age', key_added = 'kdeAge')
scp.pl.embedding_density(adata, basis='umap', key = 'kdeAge', save = 'kde-jaw-Age1.pdf')
...
for n in nodes:
    for l in lamb:
        for s in sig:
            # Ensure show_node_labels=False is passed correctly
            scf.tl.tree(
                adata2,
                method="ppt",
                Nodes=n,
...
sc.settings.set_figure_params(dpi=80, frameon=False, figsize=(12, 12))
nodes=[100, 200, 300]
lamb=[1,10,100,200,300]
#sig=[.01,.1,.2,.3,.4,.5,.6,.7,.8,.9,1]
#nodes=[ 300]
#lamb=[300]
sig=[.07]
...
for n in nodes:
    for l in lamb:
        for s in sig:
            scf.tl.tree(adata2,method="ppt",Nodes=n,use_rep="palantir",plot=True,
            device="cpu",seed=1,ppt_lambda=1,ppt_sigma=s,ppt_nsteps=100,node_fontsize=1,show_node_labels=False)
...
adata2.write('nose_Sample1.h5ad')
...
adata3 = scp.read_h5ad('jaw_Sample1.h5ad')


# In[39]:


scf.tl.curve(adata3,Nodes=30,use_rep="X_pca",ndims_rep=2,)
scf.pl.graph(adata3,basis="pca")


# In[40]:


scp.pl.pca(scp.AnnData(adata3.obsm["X_R"],obsm=adata3.obsm),color="1",cmap="Reds")


# In[41]:


pca_projections = pd.DataFrame(adata3.obsm["X_pca"],index=adata3.obs_names)
...
adata3.obsm["X_palantir"]=ms_data.values
sc.pp.neighbors(adata3,n_neighbors=30,use_rep="X_palantir")
...
adata3.obsm["X_pca2d"]=adata3.obsm["X_pca"][:,:2]
sc.tl.draw_graph(adata3,init_pos='X_pca2d')


# In[45]:


scf.tl.tree(adata3,method="ppt",Nodes=200,use_rep="palantir",
...
ckt = ctk.compute_cytotrace().compute_transition_matrix()
scp.set_figure_params(dpi=150)
scp.pl.umap(adata,color="ct_score", cmap="Spectral_r",  save = 'Fig1H-cytoTRACE.pdf', size = 10)
...
scp.pl.umap(adata, color=['ClusterAge'], title = 'Average age of cluster (week)', save = 'Fig1G-Age.pdf')
